...
index923 = np.where(T_range==923)[0][0]
index1000 = np.where(T_range==1000)[0][0]


V_Sr, delta_G, theta_list = case2(T_range, x0, x_H2O, P)
ax.plot(T_range, V_Sr/x0*100, label="Sr release")
#ax.axhline(y=V_Sr[index1000], color="black", ls="dotted")
#axinset.plot(T_range, V_Sr/x0)
ax.plot(T_range, theta_list*100, label="$\\theta_{H_2O}$", color="black")
print(x_H2O, " ", V_Sr[index1000]/x0 *100, " ", theta_list[index1000]*100, " ", T_range[index1000])



ax3.plot(T_range, V_Sr, label="x=0.4")

V_Sr, delta_G, theta_list = case2(T_range, 0.6, x_H2O, P)
ax3.plot(T_range, V_Sr, label="x=0.6")

V_Sr, delta_G, theta_list = case2(T_range, 0.8, x_H2O, P)
ax3.plot(T_range, V_Sr, label="x=0.8")

ax3.set_xlabel("T[K]")
ax3.set_ylabel("$x_{eq} = [V_{La}''']_{eq}$")
...
ax.set_ylabel("Sr release and $\\theta_{H_2O}$ in %", fontsize=14)
ax.set_title("p$_{H_2O}$=0.08 atm")
...
fig.savefig(local_path + "figs/hydrox_decomp_coverage.png", format="png", dpi=300, transparent=True)
plt.show()
