...
def case1(T_range, x=0.4, p_O2 = 0.21, p_H2O = 0.08, P=1):
    ...
    for T in T_range:
        ...
        delta_G = (E_LSCF_slab_Sr_vac_surf + 2*(E_SrO + float(sro_vibration_data[T_index_vib_data, 1])) + 2*mu_H2- E_LSCF_hydroxilated )/2
...
def case3(T_range, x=0.4, p_O2 = 0.21, p_H2O = 0.08, P=1):
    ...
    for T in T_range:
        ...
        T_index_vib_data = np.where(T_data == T)
        delta_G = (E_LSCF_single_hydrogenated + 2*(E_SrO + float(sro_vibration_data[T_index_vib_data, 1])) + mu_H2- E_LSCF_hydroxilated )/2
        delta_G_list.append(delta_G)
